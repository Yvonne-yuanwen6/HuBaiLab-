...
| 用途 | 脚本 |
|------|------|
| **批处理主程序**（含超时 + `--repair`） | `scripts/run_param_batch_step_generate.py` |
...
1. **`ocp_deep_pad`**：按层重建 octant 杆 + 大周期 pad（默认 2 mm）→ 行/层间融合，质量门控 + 单实体 remelt（禁止只靠 STEP 体积当成功）
2. **`ocp_seed_scale_zcopy`**：单胞微膨胀（≈1.005–1.02）→ **只融 iz=0 的 4×4** → **+Z 复制平移** 得 iz=1..3 → 四层 `444z` 融合（glue=off, fuzzy≈0.1）
3. seed-translate OCP：`noclip` → 扩大 `periodic_overlap_mm` → `hierarchical_batch`
4. `gmsh_fallback`

**椭圆 / 普通圆杆 OCP 444 梯子优先序**：

1. **Q=1 / 1.5（非细杆）优先 `ocp_noclip_batch64`**（验证 `af2q1_deq2_k1` / `…_k1p5` / `…_k2`）：pitch=L、不 clip，一次性 OCP batch 融全部 64 胞（`glue=shift`, `fuzzy=0.1`，约 3–5 min，QC≈64）。
...
环境变量：`FORCE=1` 强制重做；`ONLY="af2q0_deq2_k1 …"` 只跑指定案；`JOBS=2` 案级并行；`TOL_REL=0.03`；`STOP_ON_FAIL=1`；`INTERVAL=8`（监控刷新秒）。
...
| 项 | 锁定行为 | 曾出现的问题 |
|---|---|---|
| 444 优先策略 | Q≈1：**`ocp_noclip_batch64`**；其余：`ocp_seed_scale*_zcopy_*`（iz0 融 + Z 复制） | 分层/zcopy 破坏正交接触；四层各融极慢 |
...
常量入口：`scripts/run_param_batch_step_generate.py` 中 `SEED_SCALE_ZCOPY_SPECS_*`、`DEEP_PAD_SPECS_THIN_ROD`；实现：`src/export/ocp_deep_pad_array_fuse.py` → `export_ocp_noclip_batch_array_fuse` / `export_seed_scale_inflate_array_fuse`。
...
- **椭圆 (k>1) / 普通圆杆 OCP**：阵列优先 **`ocp_seed_scale_zcopy`**（微膨胀 + iz0 batch fuse + Z 复制），再回退常规 OCP glue / gmsh。
- **细杆 (deq&lt;1.75, k=1)**：优先 **deep_pad**，再 **seed_scale_zcopy** / OCP noclip。
- **单胞**：优先 **OCP `centre_stub_corner_ext`**（角端 path 延长 → 平端面；胞心 chord stub → 可融；2026-07-17 硬案验证），再 gmsh `*_both_end` / OCP `both_end_extension`。**禁止** ACCEPT：`gmsh_paper_box`（无 both_end）、裸 `gmsh_octant`、裸 OCP `centre_stub`（无 corner_ext → 杆端尖楔）。写出后 **质心 COM 归零**。Q=1 可 OCP glue；AF2/deq2/k1 可复用 `_unitcell_paper_box_cut/` 历史种子。
- **监控易误判**：`--only` 时仪表盘只显示本批案；`seed_scale` iz0 fuse 单次可数分钟无新日志，worker CPU≈100% 表示在算而非空转。`.work` 新鲜度用于标 ▶进行中。
- **并行**：`JOBS=2`（或 `--jobs 2`）可用；QC/ACCEPT 测体积不得在 worker 线程直接 `gmsh.initialize`。

**单胞策略梯子**（`export_unitcell_step_paper_box_cut` + 批处理）：  
复用种子 → gmsh pipe-first（圆）/ octant → **per-strut**（椭圆 Q≠1 时 octant 失败不再硬失败）→ OCP glue（多 strategy × fuzzy）。  
失败案重跑示例：
...
output/export/批量构型/
  ...
  _batch_sim_skipped.json        # 网格梯子耗尽后跳过的案
  af2q0_deq2_k1/
...
| 网格 | **仅 Abaqus/CAE 自动** 四面体 **C3D4**（禁止 gmsh 体网格） |
| 默认剖分 | seed **0.6 mm**，`lattice_contact`，Virtual Topology 开，~3 单元/杆径 |
...
| 资源 | 每案 **48 核 / 256 GB**；求解最多 **2 路并行**；CAE 网格 **串行** |

失败时走 **CAE 策略梯子**（仍只用 CAE）：**优先保持 seed 0.6**（换 `lattice_contact`/`lattice`/`fast`/`lattice_curve`、开关 vtopo、`seed-part-only`、`ignore-invalid`），**仅当 0.6 全部失败**才放大到 0.8 / 1.0。梯子耗尽则 **SKIP**（写入 `_batch_sim_skipped.json`）。

> **注意**：`lattice_contact` + Virtual Topology @0.6 在部分 Q≈1 BREP 上会先跑很久再报 **0 单元**。`BATCH_SIM_FORCE_REMESH=1` 时默认 `BATCH_SIM_SKIP_BASELINE=1`（跳过该 baseline，改试其它 **仍为 0.6** 的梯子档；可显式设 `0` 关闭）。`BATCH_SIM_ONLY` 列表顺序会被保留。存在 `_batch_sim_paused.json` 时队列拒绝启动（除非 `BATCH_SIM_IGNORE_PAUSE=1`）。粗 seed 成功属于不可比兜底，应在日志中核对实际 `--seed`。

求解阶段若 Abaqus **已异常退出**（如 `Excessive distortion` / `exited with errors`），默认 **不再重提**，同样写入 `_batch_sim_skipped.json`（避免崩→重提死循环）。需要强制重提时设 `BATCH_SIM_ALLOW_SOLVE_RETRY=1`。
...
环境变量：`BATCH_SIM_CPUS=48`、`BATCH_SIM_MEMORY_MB=262144`、`BATCH_SIM_MAX_PARALLEL=2`、`BATCH_SIM_ONLY="af2q0_deq2_k1 …"`、`BATCH_SIM_FORCE_REMESH=1`、`BATCH_SIM_EXPORT_ONLY=1`、`BATCH_SIM_SUBMIT_ONLY=1`、`BATCH_SIM_ALLOW_SOLVE_RETRY=1`、`BATCH_SIM_SKIP_BASELINE=1`、`BATCH_SIM_IGNORE_PAUSE=1`。
...
3. **串行 CAE** 网格其余案；失败进梯子，仍失败则 SKIP。  
4. 每导出一案立即填补空闲求解槽。
