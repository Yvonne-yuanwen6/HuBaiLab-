...
> **2026-09-15 路径变更（强制）**  
> 原机械盘仓库 `/media/art/file/XiangLang/Lattice/LWY/HuBaiLab` **已损坏停用，禁止再访问**。  
> **当前临时远程根**：`/home/art/HuBaiLab_ssd`（SSD 引导副本）。新访问路径待另行通知后再改。  
> 相关默认值已同步到 `scripts/remote_config.ps1`、`scripts/linux/hubai_env.sh`、`src/paths.py`。
...
| 服务器仓库（临时） | `/home/art/HuBaiLab_ssd`（SSD；旧机械盘路径已废弃） |
| Abaqus | `/home/art/APP/abaqus2022/Commands/abq` |
| 连接方式 | SSH（日常）；VNC 仅看桌面时用 |

> **浏览器操作**：可用 Web UI 替代部分 PowerShell 步骤（导出、提交、监控、终止、后处理）。**启动方法**见 [`Abaqus_WebUI使用说明.md`](Abaqus_WebUI使用说明.md) **§2**（开发模式：API `:8000` + 前端 `:5173`）。
...
本机 HTML 进度板 / watch_job_progress 监控
```
...
| 监控进度 | 本机 | `PS D:\HuBaiLab>`（param_batch 优先 HTML 板） |
...
**当前默认（临时）**：`/home/art/HuBaiLab_ssd`。  
**禁止**：`/media/art/file/XiangLang/Lattice/LWY/HuBaiLab`（机械盘已坏）。新路径公布前不要改回旧盘。
...
C 服务器 tmux → submit（--cpus 32 --memory-mb 131072）→ 验证 ps → Ctrl+B D
    ↓
D 本机 param_batch HTML 板 / 或扁平 watch_job_progress.ps1
    ↓
...
```bash
ls /home/art/HuBaiLab_ssd/output/export/*voxel0p8mm75_15mmin*/*.inp
```
...
详见 [`docs/Abaqus显式续算.md`](Abaqus显式续算.md)（含 s45→75% 示例、曲线合并、风险说明）。
...
#### D1（推荐）param_batch：实时 HTML 进度板

适用于服务器路径  
`output/jobs/param_batch/{CaseId}/{Slug}/`（默认 SSD：`/home/art/HuBaiLab_ssd`）。

提交后立刻：

```powershell
cd D:\HuBaiLab
$env:HU_BAI_REMOTE_ROOT = "/home/art/HuBaiLab_ssd"   # 若已在环境里可省略

# 前台（该窗口持续刷新；Ctrl+C 停止）
powershell -File scripts\start_param_batch_progress_board.ps1 -CaseId af2q1_deq2p5_k1

# 或后台（最小化 watcher，浏览器打开 HTML）
powershell -File scripts\start_param_batch_progress_board.ps1 -CaseId af2q1_deq2p5_k1 -Background
```

- 页面：`output/logs/progress_board/{CaseId}_{Slug}.html`（同目录有 `.json`；页面自动刷新）
- 核心脚本：`scripts/watch_remote_abaqus_progress.ps1`（`-CaseId` 必填）
- 远程探测：`scripts/linux/watch_sta_snapshot.sh`（缺则启动时自动 scp 一次）
- 总时钟：默认从本机 `*_meta.json` + settle 比例推 `ContactSettle+Compression`（paperbox 常为 **883.2 s**）；也可用 `-StepTotalSec`

与 Web UI「作业监控」独立：Web UI 更适合扁平 `jobs/{slug}`；**param_batch 嵌套案用本 HTML 板**。

#### D2（遗留）扁平 `jobs/{slug}`：终端进度条

**监控服务器上的扁平 job 时，必须加 `-RemoteHost` 和 `-RemoteRoot`**。脚本会每 30 s 经 SSH 拉取服务器 `.sta`，否则会读本机 `output/jobs/` 里的旧文件，进度会**卡住不动**。

```powershell
cd D:\HuBaiLab
.\scripts\watch_job_progress.ps1 `
  -RemoteHost "art@172.20.200.93" `
  -RemoteRoot "/home/art/HuBaiLab_ssd" `
...
| 本机 PowerShell #2 | 再开一个 | `PS D:\HuBaiLab>` | param_batch：`start_param_batch_progress_board.ps1`；扁平旧案：`watch_job_progress.ps1` |
...
```powershell
scp -r D:\HuBaiLab\src D:\HuBaiLab\scripts art@172.20.200.93:/home/art/HuBaiLab_ssd/
```
...
| `scripts/start_param_batch_progress_board.ps1` | **推荐**：param_batch 提交后一键开 HTML 进度板（可 `-Background`） |
| `scripts/watch_remote_abaqus_progress.ps1` | param_batch 远程 HTML 看板核心（`-CaseId` 必填；写 `output/logs/progress_board/`） |
| `scripts/linux/watch_sta_snapshot.sh` | 服务器端 `.sta`/`.lck` 快照（给上面 PS1 用） |
| `scripts/watch_job_progress.ps1` | 遗留：扁平 `jobs/{slug}` 终端进度条 + ETA（加 `-RemoteHost` / `-RemoteRoot`） |
...
- [ ] param_batch：已跑 `start_param_batch_progress_board.ps1 -CaseId ...`（或等价 `watch_remote_abaqus_progress.ps1`）
- [ ] HTML：`output/logs/progress_board/{CaseId}_{Slug}.html` 百分比 / TOTAL TIME 在更新
- [ ] 扁平旧案：`watch_job_progress.ps1` 带了 **`-RemoteHost` 和 `-RemoteRoot`**
