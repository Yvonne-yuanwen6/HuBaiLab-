...
7. **Face-mate（2026-07-19；2026-07-29 调整）**：1×1 单实体 ≠ 阵列可融。~~1×1 ACCEPT 前硬锁~~已取消——单胞门控只看单实体 + tip-sliver（strut1 配方）。pitch=L 邻胞 fuse 改由 **444 / tip-push** 阶段处理；`_seed_face_mate_ok` 仅作阵列诊断。详见 [`批量构型STEP生成说明.md`](./批量构型STEP生成说明.md) §3。
...
**批处理硬案（Q=1 hybrid，2026-07-18～19）**：分层 / zcopy 会破坏正交接触。优先 **`ocp_noclip_batch64`**：pitch=L 放置 64 胞 → 一次性 GlueShift batch（已验证 `af2q1_deq2_k1` / `…_k1p5` / `…_k2` / `af2q1_deq2p5_k1` / 椭圆 `af2q0p5_deq2_k2`；细杆 `af2q1_deq1p5_k1` 曾作 noclip 验证案，**2026-07-28 因单胞目视不合格作废**，勿再当合格样例）。裸 place 空融时用 **`ocp_scale*_batch64`**（已验证 `af2q1p5_deq2_k1p5`）。

**Face-mate（2026-07-29）**：1×1 `volume_count==1` 仍可能 pitch=L 不可贴合，但**不再**作为 1×1 ACCEPT 硬锁；对接问题放到 444 / tip-push。`_seed_face_mate_ok` 保留作阵列诊断。详见 [`批量构型STEP生成说明.md`](./批量构型STEP生成说明.md) §3。

**本批 16 案实际成功路径（2026-07-24）**：11 案 = face-mate + `noclip_batch64`；5 案早期 = gmsh / hierarchical / sequential / 整案复用。逐案表 → [`批量构型STEP生成情况明细.md`](./批量构型STEP生成情况明细.md) §2。
