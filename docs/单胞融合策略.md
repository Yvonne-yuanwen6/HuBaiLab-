...
**批处理硬案（Q=1 hybrid，2026-07-18）**：分层 / zcopy 会破坏正交接触。优先 **`ocp_noclip_batch64`**：pitch=L 放置 64 胞 → 一次性 GlueShift batch（已验证 `af2q1_deq2_k1` / `…_k1p5` / `…_k2`）。裸 place 空融时用 **`ocp_scale*_batch64`**（已验证 `af2q1p5_deq2_k1p5`）。
