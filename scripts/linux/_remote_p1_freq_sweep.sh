#!/usr/bin/env bash
# Phase 1 freq sweep from existing mph (Identity+Continuity).
# Usage: SLUG=... FREQ_MIN=5 FREQ_MAX=150 FREQ_STEP=2 bash scripts/linux/_remote_p1_freq_sweep.sh
set -euo pipefail
...
SLUG="${SLUG:-comsol_fig321_bcc_444_mesh_p1_f5_150}"
...
exec > >(tee -a "$LOG") 2>&1
echo "=== p1 sweep ${FREQ_MIN}-${FREQ_MAX} step=${FREQ_STEP} (${TOTAL} pts) $(date) ==="
