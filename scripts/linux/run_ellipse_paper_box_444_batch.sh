...
#
# Ellipse: minor axis || +Z, area = pi mm^2 (same as circle d=2 mm).
# Nominal aspect 2:1.2 scaled -> d_major ~2.582 mm, d_minor ~1.549 mm.
#
#   bash scripts/linux/run_ellipse_paper_box_444_batch.sh
#   nohup bash scripts/linux/run_ellipse_paper_box_444_batch.sh >> output/logs/ellipse_paper_box_444_batch.log 2>&1 &
#
...
LOG="${LOG:-$ROOT/output/logs/ellipse_paper_box_444_batch.log}"
...
print('$SEED_DIR/unitcell_' + slug + '_paper_box_ellipse_ellmin_eqarea.step')
"
}
...
print('$dir/' + slug + '_paper_box_ellipse_eqarea_array.step')
"
}
...
export_seeds() {
  ...
  for q in $Q_LIST; do
    ...
    if [[ "$q" == "1.0" || "$q" == "1" || "$q" == "1.5" ]]; then
      log "=== export Q=$q elliptic seed (multi-route) ==="
    ...
    log "=== export Q=$q elliptic unitcell seed (gmsh) ==="
...
")"
  ...
  if [[ ! -f "$array_step" ]]; then
    # Default fuse output name before rename
    local default_step
    default_step="$("$PY" -c "
import sys
sys.path.insert(0, '$ROOT')
...
log "=== ellipse paper_box 444 batch start Q_LIST=[$Q_LIST] ==="
export_seeds
