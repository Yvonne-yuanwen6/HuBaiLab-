#!/usr/bin/env bash
# Q0.5 C3D10M seed0.6 r3 lattice_contact elastic @ 45% strain (fast C3D10M pilot).
#
#   bash scripts/linux/run_paperbox_q05_c10m_s06r3_el_s45.sh
#   nohup bash scripts/linux/run_paperbox_q05_c10m_s06r3_el_s45.sh \
#     >> output/logs/q05_c10m_s06r3_el_s45.log 2>&1 &
set -euo pipefail
...
LOG="output/logs/q05_c10m_s06r3_el_s45.log"
VARIANT_SH="scripts/linux/run_paperbox_variant.sh"
SLUG="q05_c10m_s06r3_el_s45"
CPUS="${Q05_C10M_CPUS:-48}"
MEM="${Q05_C10M_MEMORY_MB:-262144}"
TARGET_STRAIN="${Q05_C10M_STRAIN:-0.45}"

exec > >(tee -a "$LOG") 2>&1

echo "[$(date '+%Y-%m-%d %H:%M:%S')] Q0.5 C3D10M s06r3 elastic s45 cpus=$CPUS strain=$TARGET_STRAIN slug=$SLUG"
...
bash "$VARIANT_SH" --Q 0.5 --short-slug "$SLUG" \
  --variant-suffix c10m_s06r3_el_s45 \
