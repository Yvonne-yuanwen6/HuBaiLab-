#!/usr/bin/env bash
# BCC 4×4×4 full compression: Marlow vs Mooney-Rivlin (test data) in parallel.
# Reuses baseline CAE C3D4 mesh; does not stop other running jobs.
#
#   bash scripts/linux/run_bcc_test_marlow_mr_parallel.sh
#   nohup bash scripts/linux/run_bcc_test_marlow_mr_parallel.sh \
#     >> output/logs/bcc_test_marlow_mr_parallel.log 2>&1 &
set -euo pipefail
...
mkdir -p output/logs output/reports/bcc_test_marlow_mr output/post

LOG="output/logs/bcc_test_marlow_mr_parallel.log"
READY="output/logs/bcc_test_marlow_mr_ready.json"
LOCK="$ROOT/output/logs/bcc_test_marlow_mr_parallel.lock"
exec 9>"$LOCK"
if ! flock -n 9; then
  echo "[$(date '+%Y-%m-%d %H:%M:%S')] bcc_test_marlow_mr already running (lock $LOCK)" | tee -a "$LOG"
  exit 0
fi
HEALTH_WAIT_SEC="${BCC_TEST_HEALTH_WAIT_SEC:-180}"
FIG25="data/hu_bai_tpu_fig25_tensile_traced.json"
CAD="output/cad/verified/hu_bai_bcc_af2q0_L20_4x4x4_paper_box_array.step"
BASELINE_SLUG="hu_bai_bcc_af2q0_L20_4x4x4_solid_cad_f_cae_tet0p6mm80_5mmin_paperbox"
BASELINE_MESH="output/export/${BASELINE_SLUG}/${BASELINE_SLUG}_cae_mesh.inp"

CPUS="${BCC_TEST_CPUS:-32}"
MEM="${BCC_TEST_MEMORY_MB:-196608}"
POLL_SEC="${BCC_TEST_POLL_SEC:-120}"

SLUG_MARLOW="hu_bai_bcc_af2q0_L20_4x4x4_solid_cad_f_test_marlow"
SLUG_MR="hu_bai_bcc_af2q0_L20_4x4x4_solid_cad_f_test_MR"
...
export_one() {
  ...
  rm -rf "output/export/hu_bai_bcc_af2q0_L20_4x4x4_solid_cad_f_${suffix}" \
         "output/jobs/hu_bai_bcc_af2q0_L20_4x4x4_solid_cad_f_${suffix}"
  python3 "${EXPORT_COMMON[@]}" \
    --material-model "$model" \
    --case-suffix "$suffix"
  grep -A8 '^\*Material, name=TPU' \
    "output/export/hu_bai_bcc_af2q0_L20_4x4x4_solid_cad_f_${suffix}/hu_bai_bcc_af2q0_L20_4x4x4_solid_cad_f_${suffix}.inp" \
...
path = root / "output/logs/bcc_test_marlow_mr_ready.json"
...
log "=== BCC test_marlow + test_MR parallel start cpus=$CPUS mem=${MEM}MB mass_scaling=default ==="
...
write_ready
log "=== BCC test_marlow + test_MR finished ==="
