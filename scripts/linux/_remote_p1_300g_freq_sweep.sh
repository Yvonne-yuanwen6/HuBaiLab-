#!/usr/bin/env bash
# P1 freq sweep with 300 g top payload on existing mesh_p1 mph.
# Usage: FREQ_MIN=1 FREQ_MAX=300 FREQ_STEP=5 bash scripts/linux/_remote_p1_300g_freq_sweep.sh
set -euo pipefail
...
SRC_SLUG="${SRC_SLUG:-comsol_fig321_bcc_444_mesh_p1}"
SLUG="${SLUG:-comsol_fig321_bcc_444_mesh_p1_300g_f1_300}"
...
exec > >(tee -a "$LOG") 2>&1
echo "=== P1 + ${PAYLOAD_KG}kg payload sweep ${FREQ_MIN}-${FREQ_MAX} step=${FREQ_STEP} (${TOTAL} pts) $(date) ==="
