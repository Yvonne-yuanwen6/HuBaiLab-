...
foreach ($s in $Slugs) {
    ...
    if (-not (Test-Path $odb) -or (Get-Item $odb).Length -lt 100MB) {
...
Write-Host "Done. PNG: output\reports\voxel0p8mm80_15mmin_autodt_stress_strain_compare.png" -ForegroundColor Green
