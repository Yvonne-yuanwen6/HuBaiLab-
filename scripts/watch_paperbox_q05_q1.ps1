# Live monitor: Q0.5 snap-through sweep + Q1 diagnostic sweep (scp .sta each poll).
param(
...
$Q05Jobs = @(
    ...
    @{ Label = "Q0.5 rods4";           Tag = "sfbls_af2q0p5"; Suffix = "cae_tet0p6mm80_5mmin_paperbox_paperbox_q05_rods4" },
)

$Q1Jobs = @(
    @{ Label = "Q1 settle5p";          Tag = "sfbls_af2q1"; Suffix = "cae_tet0p6mm80_5mmin_paperbox_paperbox_settle5p" },
    @{ Label = "Q1 nosettle dt1e-4";   Tag = "sfbls_af2q1"; Suffix = "cae_tet0p6mm80_5mmin_paperbox_paperbox_nosettle_dt1e4" },
    @{ Label = "Q1 settle5p dt1e-4";   Tag = "sfbls_af2q1"; Suffix = "cae_tet0p6mm80_5mmin_paperbox_paperbox_settle5p_dt1e4" },
    @{ Label = "Q1 rods4 remesh";      Tag = "sfbls_af2q1"; Suffix = "cae_tet0p6mm80_5mmin_paperbox_paperbox_q1_rods4" }
)
...
function Show-JobGroup {
    ...
    foreach ($j in $Jobs) {
        ...
        Write-Host ("  {0,-22} [{1}]" -f $j.Label, $st) -ForegroundColor $color
        if ($st -eq 'WAITING') {
            Write-Host "    (queued / not started)"
        } elseif ($st -eq 'RUNNING' -and $simS -le 0) {
            Write-Host "    packager / pre-increment"
        } else {
...
Clear-Host
Write-Host "=== Q0.5 + Q1 paperbox monitor (poll=${PollSeconds}s) ===" -ForegroundColor Cyan
...
while ($true) {
    ...
    Show-JobGroup -Title "[Q0.5 snap-through sweep]" -Jobs $Q05Jobs
    Show-JobGroup -Title "[Q1 diagnostic sweep]" -Jobs $Q1Jobs

    $q05Sweep = ssh $RemoteHost "pgrep -af run_paperbox_snapthrough_sweep.sh 2>/dev/null | grep -v grep | head -1"
    $q1Sweep = ssh $RemoteHost "pgrep -af run_paperbox_q1_diagnostic_sweep.sh 2>/dev/null | grep -v grep | head -1"
    Write-Host ("  Q0.5 queue: {0}" -f $(if ($q05Sweep) { 'RUNNING' } else { 'idle' })) -ForegroundColor DarkYellow
    Write-Host ("  Q1   queue: {0}" -f $(if ($q1Sweep) { 'RUNNING' } else { 'idle' })) -ForegroundColor DarkYellow

    $q05Done = ssh $RemoteHost "grep '^DONE ' '$RemoteRoot/output/logs/paperbox_snapthrough_sweep.log' 2>/dev/null | tail -1"
    $q1Done = ssh $RemoteHost "grep '^DONE ' '$RemoteRoot/output/logs/paperbox_q1_diagnostic_sweep.log' 2>/dev/null | tail -1"
    if ($q05Done) { Write-Host "  $q05Done" -ForegroundColor Green }
    if ($q1Done)   { Write-Host "  $q1Done" -ForegroundColor Green }

    $allQ05 = ($Q05Jobs | ForEach-Object {
        $s = "hu_bai_$($_.Tag)_L20_4x4x4_solid_cad_f_$($_.Suffix)"
        Get-VariantStatus -Slug $s
    }) -notcontains 'RUNNING' -and ($Q05Jobs | ForEach-Object {
        $s = "hu_bai_$($_.Tag)_L20_4x4x4_solid_cad_f_$($_.Suffix)"
        Get-VariantStatus -Slug $s
    }) -notcontains 'WAITING'

    $allQ1 = ($Q1Jobs | ForEach-Object {
        $s = "hu_bai_$($_.Tag)_L20_4x4x4_solid_cad_f_$($_.Suffix)"
        Get-VariantStatus -Slug $s
    }) -notcontains 'RUNNING' -and ($Q1Jobs | ForEach-Object {
        $s = "hu_bai_$($_.Tag)_L20_4x4x4_solid_cad_f_$($_.Suffix)"
        Get-VariantStatus -Slug $s
    }) -notcontains 'WAITING'

    if ($allQ05 -and $allQ1 -and $q05Done -and $q1Done) {
        Write-Host "=== all queues DONE ===" -ForegroundColor Green
        break
    }
