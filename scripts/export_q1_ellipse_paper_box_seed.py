...
Smooth-only sweep (hard requirement for paper_box / CAE):
- gmsh CorrectedFrenet / OCP frenet single-profile pipe
- OCP parallel_transport (disk/wafer 圆片) is blocked in code

  py -3 scripts/export_q1_ellipse_paper_box_seed.py --Q 1.0
  py -3 scripts/export_q1_ellipse_paper_box_seed.py --Q 1.5 --ellipse-align major
"""
...
def _seed_ok(
    ...
    *,
    ...
    if not os.path.isfile(path):
        return False
    try:
        if int(_count_seed_volumes(path)) != 1:
            return False
        _validate_seed_struts(
            path,
            L=L,
            Af=Af,
        ...
        print(f"  existing seed rejected ({exc})", flush=True)
        return False


def main() -> int:
    p = argparse.ArgumentParser(description="Elliptic paper_box unit-cell seed (smooth-only)")
    ...
    p.add_argument(
        "--ocp-only",
        action="store_true",
        help="Skip gmsh; try OCP frenet glue strategies only (smooth)",
    )
    args = p.parse_args()
    ...
    print("  ellipse sweep policy: SMOOTH ONLY (frenet / gmsh CorrectedFrenet)", flush=True)
    ...
    if _seed_ok(
        ...
        print(f"  skip (vol=1 + 8 strut corridors OK): {out_step}", flush=True)
        return 0
    ...
    # Batch glue silently keeps ~1 strut while clearing loose mass gates — never use
    # as a default ellipse route.
    ...
    def _ocp_export(
        *,
        ...
        return export_q1_ocp_glue_unitcell(
            ...
            pipe_mode=pipe_mode,
            ellipse_sweep_mode="frenet",
        )

    attempts: list[tuple[str, object]] = []
    skip_gmsh = args.skip_gmsh or args.ocp_only
    if not skip_gmsh:
        # Ellipse always uses octant sequential (force_octant in box_cut); try
        # centre-stub first, then both-end extension.
        attempts.append(("gmsh_octant_centre_stub", lambda: _gmsh_export(both_end=False)))
        attempts.append(("gmsh_octant_both_end", lambda: _gmsh_export(both_end=True)))
    if args.ocp_only:
        for strategy in _OCP_STRATEGIES:
            for fuzzy in _OCP_FUZZY:
                tag = f"ocp_frenet_{strategy}_f{fuzzy:g}"
                attempts.append(
                    (
                        tag,
                        lambda st=strategy, fz=fuzzy: _ocp_export(
        ...
        for pipe_mode, mode_tag in (
            ("centre_stub", "centre_stub"),
            ("both_end_extension", "both_end_extension"),
        ):
            attempts.append(
                (
                    f"ocp_frenet_{mode_tag}",
                    lambda pm=pipe_mode: _ocp_export(pipe_mode=pm),
                )
            )
    ...
    raise SystemExit(
        f"Q={q} elliptic seed export failed (smooth routes only):\n  " + "\n  ".join(errors)
    )
