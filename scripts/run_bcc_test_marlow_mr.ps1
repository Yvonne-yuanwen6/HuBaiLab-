# Sync, launch BCC test_marlow + test_MR on server (parallel), wait, pull CSVs, overlay plot.
param(
    [switch]$LaunchOnly,
    [switch]$PullPlotOnly,
    [int]$PollSeconds = 180
...
$RunSh = "scripts/linux/run_bcc_test_marlow_mr_parallel.sh"
$LogLocal = Join-Path $LocalRoot "output\logs\bcc_test_marlow_mr_local.log"
$ReadyRemote = "$Remote/output/logs/bcc_test_marlow_mr_ready.json"
$ReadyLocal = Join-Path $LocalRoot "output\logs\bcc_test_marlow_mr_ready.json"
...
if (-not $PullPlotOnly) {
    ...
    Log "=== launch parallel pipeline on server (background) ==="
    $cmd = "cd '$Remote' && nohup bash $RunSh >> output/logs/bcc_test_marlow_mr_parallel.log 2>&1 & echo LAUNCHED"
    ssh $Server $cmd
    if ($LaunchOnly) {
        Log "LaunchOnly — monitor: ssh $Server tail -f $Remote/output/logs/bcc_test_marlow_mr_parallel.log"
        exit 0
    }
}
...
Log "=== overlay plot ==="
py -3 (Join-Path $LocalRoot "scripts\plot_bcc_test_marlow_mr.py")
Log "=== done ==="
